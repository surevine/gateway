Feature: Upload a Release Using Maven
  ...
  Scenario: Upload a simple binary file to Nexus (with no special security metadata) using the Maven “deploy:file” target.
	...
	Then checksums are generated and the release can be downloaded by another user with access to the relevant local repository
  ...
  Scenario: Upload a simple binary file to Nexus alongside a valid security marking metadata file using the Maven “deploy:file” target.
	...
	Then checksums are generated, the security marking metadata file is present and the release can be downloaded by another user with access to the relevant local and remote repositories