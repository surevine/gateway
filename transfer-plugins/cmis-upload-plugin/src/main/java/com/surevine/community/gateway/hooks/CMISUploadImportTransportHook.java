		...
		for (final String destination : CMISProperties.listDestinations()) {
			try {
	            ...
	            try {
            		...
            		LOG.log(Level.WARNING, "Could not read from content file.", e);
	            }
			}
			catch (CmisBaseException e) {
				LOG.log(Level.WARNING, "CMIS exception when attempting to upload to destination "+destination+".  See stderr for details.", e);
			}
		}
	}
